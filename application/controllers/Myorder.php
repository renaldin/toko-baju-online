...
class Myorder extends MY_Controller
{
    ...
    public function confirmBankTransfer($invoice)
    {
        $data['order']  = $this->myorder->where('invoice', $invoice)->first();
        ...
        if (!empty($_FILES) && $_FILES['image']['name'] !== '') {
            ...
            if ($upload) {
                ...
                redirect(base_url("myorder/confirmBankTransfer/$invoice"));
            }
        }

        if (!$this->myorder->validate()) {
            $data['title']            = 'Konfirmasi Order';
            $data['form_action']    = base_url("myorder/confirmBankTransfer/$invoice");
            $data['page']            = 'pages/myorder/confirmBankTransfer';
    ...
    public function confirmDana($invoice)
    {
        $data['order']  = $this->myorder->where('invoice', $invoice)->first();
        ...
        if (!empty($_FILES) && $_FILES['image']['name'] !== '') {
            ...
            if ($upload) {
                ...
                redirect(base_url("myorder/confirmDana/$invoice"));
            }
        }

        if (!$this->myorder->validate()) {
            $data['title']            = 'Konfirmasi Order';
            $data['form_action']    = base_url("myorder/confirmDana/$invoice");
            $data['page']            = 'pages/myorder/confirmDana';
